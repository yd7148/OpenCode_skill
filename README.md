...
| `video-2x-speed` | 倍速影片處理（ffmpeg 200%／裁切黑邊，Intel QSV，支援批次） | ffmpeg + `yt-dlp` |
| `video2text` | 分析錄影影片 → 雙語 Markdown 報告 + 關鍵幀 PDF | Python venv：optimum-intel、faster-whisper、rapidocr-onnxruntime 等 |
| `yt-batch-download` | 批次下載 YouTube 影片（1080p；含直播 `--live-from-start`） | Python 3.13+、`yt-dlp`、`browser_cookie3`、ffmpeg、deno |
...
| `video-class-pipeline` | 課程影片批式分析（OCR × Whisper × 關鍵幀 PDF）與裁切/2倍速 | Python venv：EasyOCR、whisper、opencv + ffmpeg（NVENC 或 Intel QSV） |
