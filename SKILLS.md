...
**前置需求**：
- ffmpeg（`C:\Users\N000149839\opencode-tools\ffmpeg.exe`；另一台機器為 v9.0 essentials
  `C:\Users\N000149839\AppData\Local\Temp\opencode\bin\ffmpeg.exe`）
...
- **全檔 QSV 批次變體**（可背景長跑時用，2026-09-17 實測）：不做分段 concat，直接對整檔
  兩段式（video-only crop+2x → audio-only atempo → `-c copy -shortest` mux）；
  Intel UHD 770 約 11–12x realtime，13 支（約 37 小時素材）約 2.5 小時完成
- 背景啟動若回報 `Unknown: ChildProcess.kill` 其實已啟動；重下會產生第二個平行程序，
  需以 `Get-CimInstance Win32_Process -Filter "Name='python.exe'"` 確認

**產出**：`<video名>-2x.mp4`（來源旁，唯一檔名）；n8n/tvdi 課程專案用 `<名稱>_裁切2倍速.mp4`
...
- cookies 有有效期限，過期需重新匯出
- 支援直播重播（`youtube.com/live/ID`）與一般影片
- **403 兩種情況**（2026-09-17）：①「清單成功、下載才 403」＝cookies 過期，重匯即可；
  ②剛連續下載完數支後 403＝YouTube 暫時性反爬，等待數分鐘重試可解
- **仍在直播的影片**（`is_live=True`）用 `--live-from-start` 從頭錄到結束，yt-dlp 自動合併；
  錄影中只有 `.part-Frag*`，結束才出現最終 `.mp4`；此類影片要排除於 VOD 批次之外
- 批次腳本內建 skip-if-exists（>1MB 跳過）可安全重跑；檔名在主控台顯示 mojibake 屬顯示層問題，
  實際為正確 UTF-8
...
**用途**：批式處理一系列課程錄影（Google Meet / YouTube 直播畫面），產出可分析的成品（畫面 OCR、Whisper 語音轉錄、逐分鐘交叉比對、關鍵幀 PDF），並支援單次影片編輯（裁黑邊、2 倍速），以 GPU 編碼（RTX 5080 用 NVENC；Intel-only 機器用 `h264_qsv`）。
...
2. **兩 pass 分開編碼再 mux**：pass 1 僅影片（crop + `setpts=PTS/2` + `fps=30` + NVENC），pass 2 僅音訊（`atempo=2.0` + AAC），再 `-c copy -shortest` 合併——單一 pass 同時含音訊會截斷 AAC 串流（**Intel-only 機器改 `h264_qsv`，其餘相同**）
3. 用 ffprobe 確認兩串流時長皆 ≈ 來源/2，並以影格比對驗證 2x 時序

**關鍵檔案**：`E:\01-Project\2026-07-B-python_ai_tvdi\`（tvdi 課程）、`E:\01-Project\2026_08_n8n_itri\`（n8n 課程，RTX 5080），以及
`D:\80-Opnecode\Projects\2026_08_n8n_tvdi\`（n8n tvdi 場次，Intel-only 機器；見 `video-2x-speed` 的「全檔 QSV 批次變體」）
...
- **Intel-only 機器的 Workflow B**（2026-09-17，`D:\80-Opnecode\Projects\2026_08_n8n_tvdi`）：
  13 支（12 VOD + 1 `--live-from-start` 直播錄影），全部 1920x1080 30fps，裁切 `crop=1440:1080:0:0`，
  以 `h264_qsv` 兩段式輸出 `<名稱>_裁切2倍速.mp4`；吞吐約 11–12x realtime，約 37 小時素材 2.5 小時完成
